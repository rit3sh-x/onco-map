...
import type { GeneBounds, GeneDetailsFromSearch } from "@/modules/schema";
...
import { getNucleotideColorClass } from "@/utils/coloring";

interface GeneSequenceProps {
...
export function GeneSequence({
  ...
  const [localStartPosition, setLocalStartPosition] = useState(startPosition);
  const [localEndPosition, setLocalEndPosition] = useState(endPosition);
  ...
  const startDebounceRef = useRef<number | undefined>(undefined);
  const endDebounceRef = useRef<number | undefined>(undefined);

  useEffect(() => {
    setLocalStartPosition(startPosition);
  }, [startPosition]);

  useEffect(() => {
    setLocalEndPosition(endPosition);
  }, [endPosition]);

  const debouncedStartUpdate = useCallback((value: string) => {
  ...
  const debouncedEndUpdate = useCallback((value: string) => {
  ...
  const handleLocalStartChange = useCallback((value: string) => {
    setLocalStartPosition(value);
    debouncedStartUpdate(value);
  }, [debouncedStartUpdate]);

  const handleLocalEndChange = useCallback((value: string) => {
    setLocalEndPosition(value);
    debouncedEndUpdate(value);
  }, [debouncedEndUpdate]);
  ...
  const currentRangeSize = useMemo(() => {
    const start = parseInt(localStartPosition);
    const end = parseInt(localEndPosition);
    return isNaN(start) || isNaN(end) || end < start ? 0 : end - start + 1;
  }, [localStartPosition, localEndPosition]);
  ...
  useEffect(() => {
    ...
    const startNum = parseInt(localStartPosition);
    const endNum = parseInt(localEndPosition);
  ...
  }, [localStartPosition, localEndPosition, geneBounds]);
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const currentStartNum = parseInt(localStartPosition);
      const currentEndNum = parseInt(localEndPosition);

      if (isDraggingStart) {
        if (!isNaN(currentEndNum)) {
          if (currentEndNum - newPosition + 1 > maxViewRange) {
            handleLocalStartChange(String(currentEndNum - maxViewRange + 1));
          } else if (newPosition < currentEndNum) {
            handleLocalStartChange(String(newPosition));
          }
        }
      } else if (isDraggingEnd) {
        if (!isNaN(currentStartNum)) {
          if (newPosition - currentStartNum + 1 > maxViewRange) {
            handleLocalEndChange(String(currentStartNum + maxViewRange - 1));
          } else if (newPosition > currentStartNum) {
            handleLocalEndChange(String(newPosition));
          }
        }
        ...
        handleLocalStartChange(String(newStart));
        handleLocalEndChange(String(newEnd));
      }
    };
    ...
    localStartPosition,
    localEndPosition,
    handleLocalStartChange,
    handleLocalEndChange,
    maxViewRange
  ]);
    ...
    for (let i = 0; i < sequenceData.length; i += BASES_PER_LINE) {
      ...
      lines.push(
        <div key={i} className="flex">
          <div className="mr-2 text-right text-gray-500 select-none">
      ...
      <CardContent className="pb-4">
          ...
          <div className="mb-4 flex flex-col">
            <div className="mb-2 flex flex-col items-center justify-between text-xs sm:flex-row">
              ...
              <span className="text-[#3c4f3d]/70">
                Selected: {parseInt(localStartPosition || "0").toLocaleString()} -{" "}
                {parseInt(localEndPosition || "0").toLocaleString()} (
              ...
              <div className="flex flex-col space-y-4 sm:flex-row sm:items-center sm:justify-between sm:space-y-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium text-[#3c4f3d]">
                    {parseInt(localStartPosition || "0").toLocaleString()}
                  </span>
                </div>
                <div className="flex items-center gap-2">
        ...
        <div className="w-full rounded-md bg-[#e9eeea]/50 p-3">
          {isLoading ? (
            ...
            <div className="h-64 overflow-x-auto overflow-y-auto scrollbar-thin">
