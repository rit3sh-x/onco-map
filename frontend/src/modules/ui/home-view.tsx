...
import type { GeneFromSearch, GenomeAssembly } from "../schema";
...
export default function HomePage() {
  ...
  const { data: availableGenomes, isLoading: genomesLoading } = useQuery(trpc.data.getAvailableGenomes.queryOptions());

  const { data: chromosomesData, isLoading: chromosomesLoading } = useQuery(trpc.data.getGenomeChromosomes.queryOptions({ genomeId: selectedGenome }));
  const genomes: GenomeAssembly[] = availableGenomes?.genomes?.Human ?? [];

  const { data: searchResultsData, isLoading: searchLoading } = useQuery({
    ...trpc.data.searchGenes.queryOptions({
      query: searchQuery,
  ...
  const { data: browseResultsData, isLoading: browseLoading } = useQuery({
    ...trpc.data.searchGenes.queryOptions({
      query: selectedChromosome,
      genome: selectedGenome,
    }),
    enabled: mode === "browse" && Boolean(selectedChromosome),
  });

  const isLoading = genomesLoading || chromosomesLoading || searchLoading || browseLoading;

  const currentResults = mode === "search" ? searchResultsData?.results : browseResultsData?.results;
  const filteredGenes = currentResults ?? [];
  ...
  useEffect(() => {
    setError(null);
  }, [selectedGenome, selectedChromosome, searchQuery, mode]);
          ...
          <>
                ...
                {selectedGenome && (
                  <p className="mt-2 text-xs text-[#3c4f3d]/60">
                    {genomes.find((genome) => genome.id === selectedGenome)?.sourceName
                      ? genomes.find((genome) => genome.id === selectedGenome)?.sourceName
                      : <span className="inline-block h-3 w-60 animate-pulse bg-gray-300 rounded-xs"></span>}
                  </p>
                )}
              </CardContent>
            </Card>
            ...
            <Card className="mt-6 gap-0 border-none bg-white py-0 shadow-sm">
              ...
              <CardContent className="pb-4">
                  ...
                  <TabsContent value="browse" className="mt-0">
                    <div className="max-h-[150px] overflow-y-auto pr-1">
                      <div className="flex flex-wrap gap-2">
                        {chromosomesData?.chromosomes.map((chrom) => (
                          <Button
                            key={chrom.name}
                            variant="outline"
                            size="sm"
                            className={`h-8 cursor-pointer border-[#3c4f3d]/10 hover:bg-[#e9eeea] hover:text-[#3c4f3d] ${selectedChromosome === chrom.name
                              ? "text-[#3c4f3d] bg-[#e9eeea]"
                              : ""
                              }`}
                        ...
                        <TableBody>
                          {filteredGenes.map((gene, index) => (
                            <TableRow
                              ...
                              className="cursor-pointer hover:bg-[#de8246]/10"
                            >
                              <TableCell className="text-md font-medium text-[#3c4f3d]/90">
                                {gene.description}
                              </TableCell>
                              <TableCell className="text-md font-medium text-[#3c4f3d]/70">
                                {gene.name}
                              </TableCell>
                              <TableCell className="text-md font-medium text-[#3c4f3d]/70">
                                {gene.chrom}
                ...
                {!isLoading && !error && filteredGenes.length === 0 && (
